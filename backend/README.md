...
pip install -e ".[dev]"
python scripts/bootstrap_nltk.py       # downloads WordNet + tokenizers (~50 MB)
...
Copy `.env.example` to `.env` to customise. Keys: `OLLAMA_URL`, `OLLAMA_MODEL`, `DEFAULT_DIFFICULTY`, `CORS_ORIGINS`, `CHOICE_TARGET`, `OPENING_REVEAL`.
